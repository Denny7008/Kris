...
const Leaverecall = () => {
  ...
  const [selectedApplication, setSelectedApplication] = useState(null);
  const [showSuccessModal, setShowSuccessModal] = useState(false);
  const [recalledApplications, setRecalledApplications] = useState([]); // New state for tracking recalled applications
  ...
  // Handle clicking on the Recall button
  const handleRecallClick = (application) => {
    // Don't show form if already recalled
    if (recalledApplications.includes(application._id)) return;
  ...
  const onSubmit = async (data) => {
    ...
    // Add to recalled applications
    setRecalledApplications([...recalledApplications, selectedApplication._id]);
              ...
              {approvedApplications.length > 0 ? (
                approvedApplications.map((application, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <button
                        onClick={() => handleRecallClick(application)}
                        className={`${
                          recalledApplications.includes(application._id)
                            ? "bg-gray-500 cursor-default"
                            : "bg-red-500 hover:bg-red-600"
                        } text-white py-1 px-4 rounded-lg`}
                        disabled={recalledApplications.includes(application._id)}
                      >
                        {recalledApplications.includes(application._id) ? "Recalled" : "Recall"}
                      </button>
                    </td>
                  </tr>
                ))
