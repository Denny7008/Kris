  ...
  useEffect(() => {
    const fetchUserDataAndKPIs = async () => {
      try {
        ...
        const fetchedJobTitle = userData?.jobTitle;
        ...
        const { data: allTodos } = await axios.get(`http://localhost:5000/todo/user/${userId}`);
        console.log("All KPIs:", allTodos);
  
        // Filter KPIs where the user ID matches
        const userTodos = allTodos.filter((kpi) => kpi.user._id === userId);
        setTodos(userTodos.map((kpi) => kpi.title));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {todos.length > 0 ? (
            todos.map((todo, index) => (
              <li key={index} className="bg-blue-100 text-blue-700 px-4 py-2 rounded-lg">
                {todo}
              </li>
            ))
          ) : (
