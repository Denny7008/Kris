...
const PayslipSection = () => {
    ...
    const fetchUserAndPayslips = async () => {
      try {
        ...
        if (payslipData.success) {
          ...
          // Extract unique years from payDate
          const uniqueYears = [
            ...new Set(payslipData.payslips.map((p) => new Date(p.payDate).getFullYear().toString()))
          ];
  ...
  useEffect(() => {
    if (selectedYear) {
      const availableMonths = payslips
        .filter((p) => new Date(p.payDate).getFullYear().toString() === selectedYear)
        .map((p) => p.period.trim()); // Extract month
  ...
  useEffect(() => {
    if (selectedMonth && selectedYear) {
      const foundPayslip = payslips.find(
        (p) => p.period === selectedMonth && new Date(p.payDate).getFullYear().toString() === selectedYear
      );
      setSelectedPayslip(foundPayslip || null);
    }
